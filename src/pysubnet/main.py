...
def generate_keys(account_key_type: AccountKeyType):
    """Generate keys with rich output"""
    with console.status("[bold green]Generating keys for nodes...[/bold green]") as status:
        for node in NODES:
            console.print(Panel.fit(
                f"[bold cyan]Setting up {node['name']}[/bold cyan]",
                subtitle=f"[dim]Location: {node['base_path']}[/dim]"
            ))
            ...
            console.print("\t[dim]Libp2p node key:[/dim] [cyan]{0}[/cyan]".format(node["libp2p-public-key"]))
            ...
            console.print("\t[dim]Aura public key (ss58):[/dim] [green]{0}[/green]".format(aura["ss58_address"]))
            ...
            console.print("\t[dim]Grandpa public key (ss58):[/dim] [yellow]{0}[/yellow]".format(grandpa["ss58_address"]))
            ...
            match account_key_type:
                case AccountKeyType.AccountId20:
                    ...
                    console.print(
                        "\t[dim]Validator AccountId20 (ecdsa) public-key:[/dim] [magenta]{0}[/magenta]".format(
                ...
                case AccountKeyType.AccountId32:
                    ...
                    console.print(
                        "\t[dim]Validator AccountId32 (sr25519) public-key:[/dim] [blue]{0}[/blue]".format(
...
def insert_keystore(chainspec: str):
    ...
    with Progress() as progress:
        task = progress.add_task("[cyan]Inserting keys into keystore...", total=len(NODES)*2)
        ...
        for node in NODES:
            ...
            progress.update(task, advance=1, description=f"[cyan]Inserting AURA keys for {node['name']}")
            ...
            progress.update(task, advance=1, description=f"[cyan]Inserting Grandpa keys for {node['name']}")
...
def setup_dirs():
    ...
    if len(os.listdir(ROOT_DIR)) > 0:  # ROOT_dir is not empty
        if INTERACTIVE:
            console.print(f"[yellow]⚠ Warning:[/yellow] Root directory [cyan]{ROOT_DIR}[/cyan] is not empty.")
    ...
    with console.status("[cyan]Creating node directories...[/cyan]") as status:
        for node in NODES:
            ...
            console.print(f"\t[dim]Created directory for[/dim] [cyan]{node['name']}[/cyan]")
...
def init_bootnodes_chainspec(chainspec: str) -> Path:
    ...
    console.print(Panel.fit(
        ...
        subtitle=f"[dim]Using template: {chainspec}[/dim]"
    ))
    ...
    console.print(Panel.fit(
        f"[green]Chainspec generated successfully[/green]",
        subtitle=f"[dim]Location: {chainspec_path}[/dim]"
    ))
...
def generate_raw_chainspec(chainspec: Path) -> Path:
    ...
    try:
        ...
        console.print(Panel.fit(
            f"[green]Raw chainspec generated[/green]",
            subtitle=f"[dim]Location: {raw_chainspec_path}[/dim]"
        ))
...
def display_network_status():
    ...
    for node in NODES:
        ...
        table.add_row(
            node["name"],
            log_path,
            f"[link={explorer_link}]{explorer_link}[/link]"
        )
...
def start_network(config: CliConfig):
    ...
    with Progress() as progress:
        ...
        for i, node in enumerate(NODES):
            ...
            progress.update(task, advance=1, description=f"[cyan]Starting {node['name']}...")
            console.print(f"\t[dim]Started {node['name']} (PID: [yellow]{p.pid}[/yellow])[/dim]")
...
def main():
    ...
    # Print header
    console.print(Panel.fit(
        "[bold blue]PySubnet Network Manager[/bold blue]",
        subtitle="[dim]A tool for managing Substrate test networks[/dim]"
    ))
    ...
    console.print(f"[dim]Account key type:[/dim] [magenta]{config.account_key_type.value}[/magenta]")
    console.print()
    ...
    if INTERACTIVE and not config.poa:
        proceed = Confirm.ask(
            "Does your node only require Aura/Grandpa authorities (Proof-of-Authority)?\n"
            "[dim]Select 'no' if you're using a custom network configuration[/dim]",
            default=False
        )
        if proceed:
